...
"""
wp_mark.py
Connects to the rover via MAVLink (udp:127.0.0.1:14550) and, on each
MISSION_ITEM_REACHED (waypoint reached), performs the sequence:
...
def wait_heartbeat(master: mavutil.mavfile):
    print("[wp_mark] Waiting for heartbeat...", flush=True)
...
def send_do_set_servo(master: mavutil.mavfile, servo_no: int, pwm: int):
    try:
        master.mav.command_long_send(
            ...
            0,
            float(servo_no),  # param1: servo number
            float(pwm),       # param2: PWM value
            0, 0, 0, 0, 0,
        )
        print(f"[wp_mark] DO_SET_SERVO ch={servo_no} pwm={pwm}", flush=True)
    except Exception as e:
        print(f"[wp_mark] ERROR sending DO_SET_SERVO: {e}", flush=True)


def main():
    global running
    print("[wp_mark] Starting (connecting to MAVLink)...", flush=True)
    master = None
