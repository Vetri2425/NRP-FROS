...
export default function ConfigEditor({ selectedMode, JETSON_API, status, onRefresh }: Props) {
  ...
  const saveConfig = async (): Promise<boolean> => {
    try {
      ...
      await showAlert('Config Updated', 'Config updated!');
      return true;
    } catch (e: any) {
      await showAlert('Config Update Failed', e?.message || 'Failed to update config');
      return false;
    } finally {
      setSaving(false);
    }
  ...
  const saveAndRestart = async () => {
    ...
    try {
      ...
      await showAlert('Mode Restarted', data.status || `Restarted ${selectedMode}`);
    } catch (e: any) {
      await showAlert('Mode Restart Failed', e?.message || `Failed to restart ${selectedMode}`);
    } finally {
      setRestarting(false);
      onRefresh && onRefresh();
    }
