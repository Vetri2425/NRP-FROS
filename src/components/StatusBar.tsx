...
import { Wifi, WifiOff, Radio, Battery, Cable, Gamepad2 } from 'lucide-react';
...
const StatusBar: React.FC = () => {
  ...
  React.useEffect(() => {
    console.log('[DATA FLOW] 📶 StatusBar - Connection state changed:', connectionState);
  }, [connectionState]);
  ...
  // RC (Remote Control) Status
  const getRCStatus = () => {
    // RC is only connected if the backend is connected
    const isConnected = connectionState === 'connected';
    return {
      connected: isConnected,
      color: isConnected ? 'text-green-400' : 'text-red-400',
      tooltip: isConnected ? 'RC: Connected' : 'RC: Disconnected',
    };
  };
      ...
      <div className="status-bar-panel">
        ...
        <div className="status-bar-content">
          <div className="flex items-center gap-5">
            ...
            {/* RC (Remote Control) */}
            <Tooltip text={rcStatus.tooltip}>
              <div className={`${rcStatus.color} transition-colors`}>
