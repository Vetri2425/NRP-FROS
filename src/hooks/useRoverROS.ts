...
const services = useMemo<RoverServices>(
  () => ({
    ...
    uploadMission: (waypoints: Waypoint[]) => {
      // Send lat/lng as decimal coordinates with 7 decimal precision
      // Backend expects: lat between -90 and +90, lng between -180 and +180
  ...
  const roverPosition = useMemo(() => {
    ...
    // Skip {0, 0} default position to prevent initial map jump
    if (lat === 0 && lng === 0) {
      return null;
    }
    ...
    // Log only when position actually changes (not on every memoization check)
  ...
  return {
    ...
    roverPosition, // ← Add this new property
    connectionState,
    reconnect,
    services,
    onMissionEvent,
