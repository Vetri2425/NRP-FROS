...
const PolygonTool: React.FC<PolygonToolProps> = ({ onGenerate, onClose }) => {
  ...
  const handleGenerate = async () => {
    ...
    if (points.length < 3) {
      await showAlert("Invalid Polygon", "A polygon must have at least 3 vertices.");
      return;
    }
    ...
    if (invalid) {
      await showAlert("Invalid Coordinates", "Invalid coordinates detected. Please check your input.");
      return;
    }
    ...
    try {
      ...
      if (waypoints.length === 0) {
        await showAlert("Generation Failed", "No waypoints generated. Check polygon size and spacing.");
        return;
      }
      ...
      await showAlert("Generation Error", "Error generating polygon mission. Check coordinates or spacing.");
    }
  };
